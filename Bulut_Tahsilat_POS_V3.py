...
melted_df = pd.melt(POS_Ozet, id_vars=["Banka", "Firma IBAN", "Tarih"], value_vars=["Banka1", "Pos", "Banka2", "Komisyon"], 
                    var_name="Type", value_name="Tutar")
mapping = {'Banka1': 1, 'Pos': 2, 'Komisyon': 3, 'Banka2': 4}

melted_df["Sort"]= melted_df['Type'].map(mapping)
melted_df.sort_values(by=["Banka", "Tarih", "Sort"], inplace=True)
...
melted_is = pd.melt(IS_Pos, id_vars=["Banka", "Firma IBAN", "Tarih"], value_vars=["Banka1", "Pos", "Komisyon"], 
                    var_name="Type", value_name="Tutar")
mapping = {'Banka1': 2, 'Pos': 1, 'Komisyon': 3}

melted_is["Sort"]= melted_is['Type'].map(mapping)
melted_is.sort_values(by=["Banka", "Tarih", "Sort"], inplace=True)
